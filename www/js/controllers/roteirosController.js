var roteirosController = function($scope, $http) {
    ...
    var loadRoteiros = function(loadRoteirosCallback) {
        ...
        var lastUpdate = localStorage["sync_roteiros"], forceUpdate = false;
        if (lastUpdate) {
            var today = new Date();
        ...
        $localData.findAll($http, "roteiros", forceUpdate, function(response) {
            ...
            for (var i in $scope.listItens) {
                ...
                if (tabelas.indexOf(item.tabela) == -1) {
                    ...
                    $localData.findAll($http, "produtos", false, null, "tabela=" + item.tabela, "produtos-" + item.tabela);
                }
            }
    ...
    $scope.sendModels = function(type, apiModel, success) {
        ...
        var send = function(callback) {
            ...
            $http.post(API_URL + "/" + type + ".json", data + "&user_email=" + localStorage["user_email"] + "&user_token=" + localStorage["user_token"]).success(function() {
